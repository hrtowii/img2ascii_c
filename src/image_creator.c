...
void draw_char(unsigned char *buf, int x, int y, char c, struct color color,
               int width, int height, int width_in_bytes) {
...
int create_image_from_ascii(struct ascii_character **character,
  ...
  for (int row = 0; row < image_height; row++) {
    for (int col = 0; col < image_width; col++) {
      ...
      draw_char(buf, x, y, character[row][col].brightness,
                character[row][col].color, out_width, out_height,
                width_in_bytes);
    }
  }
